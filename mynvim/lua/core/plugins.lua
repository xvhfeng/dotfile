...
plugins_configure.setup = function()
    ...
    for gidx, plugins_group in ipairs(plugins_configure.plugins_groups) do
        ...
        for idx, plugin in ipairs(plugins) do
            ...
            xlog.trace("plugin:%s idx:%d [with %s] in group:%s load plugin_path:%s", plugin_name, idx, plugin_desc,
                group_name, plugin_path)
            ...
            if not ok then
                xlog.error("plugin:%s load failed and skip it try next one", plugin_name, plugin_desc, group_name,
                    plugin_path)
                xlog.error(plug)
                goto continue_2
            end
            ...
            xlog.trace("plugin:%s [with %s] in group:%s success in loading plugin_path:%s", plugin_name, plugin_desc,
                group_name, plugin_path)

            local in_enable = core["enable"]
            if opt.isRealFalse(in_enable) then
                xlog.trace("plugin:%s [with %s] in group:%s is enable=false in plugin core loading by plugin_path:%s",
                    plugin_name, plugin_desc, group_name, plugin_path)
                goto continue_2
            end
...
plugins_configure.create_mapping = function()
    ...
    for gidx, plugins_group in ipairs(plugins_configure.plugins_groups) do
        ...
        for _, plugin in ipairs(plugins) do
            ...
            xlog.trace("plugin:%s [with %s] in group:%s load plugin_path:%s", plugin_name, plugin_desc, group_name,
                plugin_path)
            ...
            if mapping then
                if type({}) == type(mapping) then
                    ...
                    -- key_mapping()
                    xlog.trace("plugin:%s [with %s] in group:%s success in make keymapping plugin_path:%s", plugin_name,
                        plugin_desc, group_name, plugin_path)
                else
                    mapping()
                end
            end
