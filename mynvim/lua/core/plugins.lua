...
plugins_configure.plugins_groups = { -- {{{
	[1] = {
		...
		["plugins"] = {
			...
			{ name = "dressing", enable = true, desc = "更好看的输入框" },
			...
			{ name = "indent-blankline", enable = true, desc = "展现缩进插件,只在nvim中使用" },
			...
			{ name = "remember", enable = true, desc = "打开文件的时候,回到上一次编辑的地方" },
			{ name = "toggleterm", enable = true, desc = "虚拟终端，执行系统bash与lazydocker" },
			--[[
            {
                name = "noice",
                enable = true,
			...
			{ name = "bigfile", enable = true, desc = "解决大文件卡顿" },
	...
	[2] = {
		...
		["plugins"] = {
			...
			-- { name = "text-case", enable = true, desc = "大词大小写" },
	...
	[3] = {
		...
		["plugins"] = {
			...
			-- { name = "cybu", enable = true, desc = "buffer的资源管理器" },
			...
			{ name = "windowpicker", enable = true, desc = "vim分屏的时候,快速选择window" },
	...
	[4] = {
		...
		["plugins"] = {
			...
			{ name = "grug-far", enable = true, desc = "神器 查找或者是替换的插件。" },
			{ name = "fzf", enable = true, desc = "查找操作" },
			-- { name = "narrow", enable = true, desc = "自动删除行尾空格" },
	...
	[5] = {
		...
		["subpath"] = "sltdo",
		["plugins"] = {
			{ name = "nvim-hlslens", enable = true, desc = "高亮匹配且跳转" },
			...
			{ name = "vim-asterisk", enable = true, desc = "不移动鼠标通过*/#选择文本" },
			{ name = "vim-matchup", enable = true, desc = "使用%查找对应的范围标签" },
			{ name = "vim-visual-multi", enable = true, desc = "多光标插件" },
	...
	[6] = {
		...
		["plugins"] = {
			...
			{ name = "goto-preview", enable = true, desc = "lsp预览插件" },
			-- { name = "symbols", enable = true, desc = "使用LSP在nvim中树状展现一个符号的outline" },
	...
	[7] = {
		...
		["plugins"] = {
			...
			{ name = "todo", enable = true, desc = "代码中todo" },
			--{
			--    name = "vimwiki",
			--    enable = false,
	...
	[9] = {
		...
		["plugins"] = {
			...
			--    desc = "管理docker的工具",
			--},
			--{ name = "lazydocker", enable = false, desc = "管理lazydocker的UI" },
		},
	},
	[10] = {
		["name"] = "code",
...
plugins_configure.setup = function()
	...
	for gidx, plugins_group in ipairs(plugins_configure.plugins_groups) do
		...
		xlog.trace("group:%s plugin namespace:%s plugin count ->%d", group_name, namespace, #plugins)
		...
		for idx, plugin in ipairs(plugins) do
			...
			xlog.trace("plugin name -> %s ,enbale ->%s desc ->%s", plugin_name, plugin_enable, plugin_desc)
			...
			if not ok then
				xlog.error(
					"plugin:%s load failed and skip it try next one",
					plugin_name,
					plugin_desc,
					group_name,
					plugin_path
...
plugins_configure.create_mapping = function()
	...
	for gidx, plugins_group in ipairs(plugins_configure.plugins_groups) do
		...
		for _, plugin in ipairs(plugins) do
			...
			xlog.trace(
				"plugin:%s [with %s] in group:%s load plugin_path:%s",
				plugin_name,
				plugin_desc,
				group_name,
				plugin_path
