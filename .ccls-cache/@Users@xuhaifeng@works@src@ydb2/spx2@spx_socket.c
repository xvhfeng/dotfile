...
spx_private err_t spx_socket_keepalive(int fd, bool_t enable, size_t alive_timeout, size_t detect_times, size_t detect_timeout);
...
spx_private err_t spx_socket_keepalive(int fd, bool_t enable, size_t alive_timeout, size_t detect_times, size_t detect_timeout) {
    ...
    //open the keepalive
    int optval = 1;
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &optval, (socklen_t) sizeof(int))) {
        err = errno;
        return err;
    }
    if (-1 == setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, &alive_timeout, sizeof(size_t))) {
        err = errno;
        return err;
    }
    if (-1 == setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &detect_times, sizeof(size_t))) {
        err = errno;
        return err;
    }
    if (-1 == setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, &detect_timeout, sizeof(size_t))) {
        err = errno;
        return err;
    }
#endif
...
spx_private err_t spx_socket_linger(const int fd, bool_t enable, size_t timeout) { /*{{{*/
    ...
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_LINGER, &optval, (socklen_t) sizeof(struct linger))) {
        return errno;
    }
    return 0;
} /*}}}*/
...
spx_private err_t spx_socket_timout(int fd, size_t timeout) { /*{{{*/
    ...
    waittime.tv_usec = 0;
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &waittime, sizeof(struct timeval))) {
        return errno;
    }
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &waittime, sizeof(struct timeval))) {
        return errno;
    }
    return 0;
} /*}}}*/
...
err_t spx_socket_start(const int fd,
                       ...
                       const int port,
                       bool_t is_keepalive,
                       size_t alive_timeout,
                       size_t detect_times,
                       size_t detect_timeout,
                       bool_t is_linger,
                       size_t linger_timeout,
                       bool_t is_nodelay,
                       bool_t is_timeout,
                       size_t timeout,
                       size_t listens) {
    ...
    if (0 != (err = spx_socket_keepalive(fd, is_keepalive, alive_timeout, detect_times, detect_timeout))) {
        return err;
    }
